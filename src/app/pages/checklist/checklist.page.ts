...
export class ChecklistPage implements OnInit {
  ...
  constructor(
    ...
    FirebaseMessaging.addListener("notificationReceived", async (event) => {
      ...
      audio.onended = async () => {
        ...
        that.router.navigate(['/job-review-cos'], { 
          replaceUrl: false, 
          queryParams: { id: that.id } 
        }).then(() => {
  ...
  async loadDataLoading() {
    ...
    Object.entries(arr1).forEach(([key, value]) => {
      this.util.denom_kertas.map(i => { 
        if(i.id==key) {
            return { 
              item: i.item, 
              value: i.value = (value == "" ? "0" : value), 
              // total: i.total = (Number(i.item) * Number(value)).toLocaleString('id-ID'), 
              total: i.total = (Number(i.id) * Number(value)).toLocaleString('id-ID'), 
              status: i.status = (i.value !== "" && value !== 0 ? '✓' : '')
            }
        } else {
          return { 
            item: i.item, 
            value: i.value = (i.value=="" ? "0" : i.value),  
            total: i.total = (Number(i.id) * Number(i.value)).toLocaleString('id-ID'), 
            status: i.status = (i.value !== "" && i.value !== "0" ? '✓' : '')
          }
        }
      });
    });
    ...
    this.jumlah_lembar = this.util.denom_kertas.reduce((sum, item) => {
      return sum + Number(item.value || '0');
    }, 0);

    this.jumlah_logam = this.util.denom_logam.reduce((sum, item) => {
      return sum + Number(item.value || '0');
    }, 0);
  ...
  checkboxOnCHange($event: any, i: any, type: any = 'kertas') {
    ...
    if (type == "kertas") {
      this.util.denom_kertas[i].status = $event.target.checked == true ? "✓" : "";
      ...
      this.util.denom_logam[i].status = $event.target.checked == true ? "✓" : "";
  ...
  checkboxOnCHangeCTC($event: any, i: any, type: any = 'kertas') {
    ...
    if (type == "kertas") {
      this.util.denom_kertas_ctc[i].status = $event.target.checked == true ? "✓" : "";
      ...
      this.util.denom_logam_ctc[i].status = $event.target.checked == true ? "✓" : "";
  ...
  async onDownload() {
    const alert = await this.alertController.create({
      header: 'Down✓!',
      message: 'Your E-Receipt is Down✓',
      mode: "ios",
...
// `);
  ...
  onJobCompleteBranch() {
    ...
    if(this.data_loading[0].group=="Retail") {
      body.set('need_review', 'true');
    }
  ...
  async openDenominationPopup(type: 'kertas' | 'logam' = 'kertas') {
    ...
    modal.onDidDismiss().then((data) => {
      ...
      if(json.jenis=='kertas') { 
        // Update denom_kertas with the new value and total for the selected denomination
        this.util.denom_kertas = this.util.denom_kertas.map(i => {
          if (i.id == json.denom) {
            const value = json.value;
            const total = (Number(json.denom) * Number(value)).toLocaleString('id-ID');
            const status = value == '0' ? '' : '✓';
            return { ...i, value, total, status };
          }
          return i;
        });
        ...
        // Update denom_logam with the new value and total for the selected denomination
  ...
  async ReviewPage() {
    ...
    if (this.total == null || this.total == 0) {
      ...
      toast.message = 'Total tidak boleh 0';
      toast.duration = 2000;
      toast.color = 'danger';
    ...
    this.util.navigateToPage('job-review', param);
  }
  ...
  async ReviewPageCOS() {
    ...
    if (this.total == null || this.total == 0) {
      ...
      await toast.present();
      return;
    } else if (this.total>2000000000) {
      const toast = document.createElement('ion-toast');
      toast.position = 'middle';
      toast.message = 'Total melebihi threshold 2 Miliar';
      toast.duration = 2000;
      toast.color = 'danger';
      document.body.appendChild(toast);
      await toast.present();
    ...
    //   }
    // };
    // this.util.navigateToPage('job-review-cos', param);
    this.countdownAlert();
